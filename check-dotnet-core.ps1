...
if($env:isVs2017 -ne 'true') {
  # VS 2015
  if($netCoreRuntime.Length -ne 13) { throw 'Wrong number of .NET Core runtimes installed' }
...
if($env:isVs2017 -ne 'true') {
  # VS 2015
  if($netCoreSDK.Length -ne 15) { throw 'Wrong number of .NET Core SDKs installed' }
  ...
  if($netCoreSDK[13].Name -ne '2.1.4') { throw '.NET Core SDK 2.1.4 not found'}
} else {
  # VS 2017
  if($netCoreSDK.Length -ne 17) { throw 'Wrong number of .NET Core SDKs installed' }
