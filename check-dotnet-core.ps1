...
if($netCoreRuntime[4].Name -ne '1.1.0') { throw '.NET Core 1.1.0 not found'}
if($netCoreRuntime[5].Name -ne '1.1.1') { throw '.NET Core 1.1.1 not found'}

$netCoreSDK = (Get-ChildItem 'C:\Program Files\dotnet\sdk')
if($netCoreSDK.Length -ne 5) { throw 'Wrong number of .NET Core SDKs installed' }
